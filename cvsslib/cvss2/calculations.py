...
def calculate_base_score(run_calculation):
    # BaseScore = round_to_1_decimal(((0.6*Impact)+(0.4*Exploitability)-1.5)*f(Impact))
    # f(impact)= 0 if Impact=0, 1.176 otherwise
    impact = run_calculation(calculate_impact)
...
def calculate_environmental_score(run_calculation,
    ...
    adjusted_temporal = run_calculation(calculate_adjusted_impact)
    adjusted_temporal = run_calculation(calculate_temporal_score, adjusted_temporal)

    return round(
        (adjusted_temporal + (10 - adjusted_temporal) * collat_damage.value) * target_dist.value, 1
    )
